...
class Estimator:
    ...
    def save_checkpoint(self, save_optim=False, name=None) -> None:
        """Save checkpoint if internal ``saver`` is not `None`.

        Args:
            save_optim (bool): whether to save optimizer in chackpoint.
            name (str): if `None`, the `net-{epoch}.pt` will be used for chkpoint filename.
        """
        if not hasattr(self, 'saver') or self.saver is None:
            return
        optim = self.optimizer if save_optim else None
        if name:
            self.saver.save_named(self.model, optim, name)
        else:
...
def epoch_tensorboard_logging_handeler(estmt):
    """Tensorboard logging for epoch of scalar and histogram."""
    if not hasattr(estmt, 'logger') or estmt.logger is None:
        return

    estmt.logger.scalar(estmt.history.metric, estmt.state.epoch)
    estmt._log.debug('tensorboard_epoch_logging logs scalars')

    if estmt._hist_dict:
        kw_histograms = {tag: torch.cat(tensors) for tag, tensors in estmt._hist_dict.items()}
        estmt.logger.histogram(kw_histograms, estmt.state.epoch)
        estmt._hist_dict = defaultdict(list)
        estmt._log.debug('tensorboard_epoch_logging logs histograms')


def minibatch_tensorboard_logging_handeler(estmt):
    """Tensorboard logging for minibatch of histogram and image.
    ...
    image, histogram = summary.get('image'), summary.get('histogram')

    estmt.logger.image(image, estmt.state.epoch, prefix)
    estmt._log.debug('tensorboard_logging logs images')

    if histogram:
        for tag, tensor in histogram.items():
            estmt._hist_dict[f'{prefix}{tag}'].append(tensor.clone())
        estmt._log.debug('tensorboard_logging accumulate histograms')
...
def checkpoint_handeler(estmt):
    """Save the checkpoints."""
    # save the checkpoint with best score
    score = estmt.history.pop('_score')
    if score and score > estmt.state.best_score:
        estmt.state.best_score = score
        estmt.save_checkpoint(name='best')

    # checkpoint saving routine
    estmt.save_checkpoint()


class State(AttrDict):
    """State for the communication between estimator and extensions."""

    def __init__(self, epoch=0):
        """Init with starting epcoh."""
        super().__init__()
        self.epoch = epoch
        self.best_score = 0
...
class OneEstimator(EstimatorEventMixin, Estimator):
    ...
    def add_default_event_handlers(self):
        self.add_event_handler(Events.ITERATION_END, minibatch_tensorboard_logging_handeler)
        self.add_event_handler(Events.EPOCH_END, epoch_tensorboard_logging_handeler)
        self.add_event_handler(Events.EPOCH_END, checkpoint_handeler)
    ...
    def evaluate(self, data_loader, inference_fn, longtime_pbar=False):
        ...
        with torch.no_grad():
            for data in progress:
                ...
                # the `score` metric for other auto-functions
                score = result.pop('score')
                self.history.update({'_score': score})
